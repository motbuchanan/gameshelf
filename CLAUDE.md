...
*Last verified: 2026-06-14 (later) · runtime `gamenight.js?v=12` · SW `gameshelf-v16` · 33 games + 2 puzzle books · vs-Computer in the picker · MULTIPLAYER (2-4, humans+computers) via GN.partyStart · One Card Left! card game with house STACKING toggle · 5 RETRO ARCADE games + a per-player ACHIEVEMENTS system + a HIGH SCORES book · (early history below says 19 games / ?v=6 — living log is at the BOTTOM)*

> **2026-06-14 session (see GAME_SHELF_HANDOFF.md "State NOW" for the full writeup):** Added 5 house arcade games (Snake, Paddle Duel, Brick Break, Blocks, Star Defender) — single-file canvas, wired into index.html + sw.js. Added a per-player ACHIEVEMENTS system: `achievements.js` engine (tiered played/wins/challenge ladders, tune via constants up top), wired into `GN.recordGame` via `GN._award`/`GN._ceremony` (try/catch-safe), participants captured in `GN._participants` (matchStart/partyStart) + `GN.soloStart` who's-playing picker for the 4 solo arcade games, an Achievements VIEW (achievements.html) and a single-holder HIGH SCORES book (highscores.html). achievements.js auto-loads on every page via a loader appended to gamenight.js. Also fixed a bot-freeze (dots/connect4/mancala were missing bot.js). KNOWN GAP: badges/top-scores are per-device (no Firebase mirror yet); ledger titles (Most Wins/Coin Flips) do sync. shelf2.html "cabinet" redesign is concept-only, excluded from the zip.
...
## Update — 2026-06-14 (audit + bugfix pass, headless playtesting)
- AUDIT METHOD: jsdom rig that actually boots & plays each game (canvas + audio + Web-Animations stubs, a controllable virtual clock, getBoundingClientRect mapped to canvas size so taps hit real cells). Static: all 44 inline + 10 standalone scripts pass node --check; every index link / sw precache entry resolves; all 42 pages boot with 0 load errors.
- ENGINES re-verified independently: One Card Left! engine 9,000 games (2–4p, stacking on/off, easy/med/hard) — 108-card conservation held every step, all terminated with a valid winner, 0 failures. Chess: perft 17/17 across 5 standard positions + mate/stalemate/insufficient-material/check detection. Connect 4 vs Computer: 12 full games through the real picker→coin-flip→bot pipeline, bot moved every time. Achievements engine: 12/12 unit + end-to-end ceremony fires. Bots present & actually moving in all 8 bot games (mancala sowed in all CPU-first games).
- BOARD/CARD playtest: all 25 driven with real interaction (cells, canvas taps, action buttons, pass-device curtains; sorry via its own #startBtn+coin, backgammon via roll+taps) — ZERO JS errors. Caveat: this proves robustness (no crashes/soft-locks, games start→progress→finish), not exhaustive rules-correctness; rule-level proof exists only for the separable engines (OCL, chess). Backgammon got the shallowest random coverage.
- BUG #1 (FIXED) brickbreak.html newGame: cancelled G.raf but never reset it, then only restarted `if(!G.raf)loop()` → loop dead on replay (ball frozen after "Play again"). Fix: `cancelAnimationFrame(G.raf);G.raf=0;` + unconditional `draw();loop();` (matches blocks/stardefender). Verified: replay ball moves again.
- BUG #2 (FIXED) snake.html setDir: same-direction/no-reverse guard returned BEFORE the start-the-loop code, so a first press of forward (Right / swipe-right) did nothing. Fix: start the run on any first input, then apply the no-reverse/no-repeat guard. Verified: forward-only first input now starts the snake.
- No runtime/CACHE bump (HTML-only changes; precache list unchanged). Upload just snake.html + brickbreak.html (or this full zip). shelfmodel.html (categorized-cabinet redesign) was attempted and REJECTED — flat, lost the cover-art/cardstock/gloss character; correct approach is to reorganize the real index.html (keeping its art+materials), not rebuild from scratch. Not in this zip.
